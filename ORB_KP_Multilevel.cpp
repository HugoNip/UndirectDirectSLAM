...
/**
 * generate the descriptor for ORB based features
 ...
 * @param descriptors
 */
void ComputeORBMultiLayer(const cv::Mat &img,
...
/**
 * Brute-Force Matcher
 * @param desc1 the reference descriptor
 * @param desc2 the second descriptor
 * @param matches machers
 */
 ...
 * @param matches
 */
void BfMatchMultiLayer(const std::vector<std::vector<DescType>> &desc1,
...
int main(int argc, char** argv) {
    // Read images
    cv::Mat img_1 = cv::imread(image_file1, 0);
    cv::Mat img_2 = cv::imread(image_file2, 0);
    ...
    // reference image
    std::vector<std::vector<cv::KeyPoint>> keypoints_1;
    std::vector<std::vector<DescType>> descriptors_1;
    ComputeORBMultiLayer(img_1, keypoints_1, descriptors_1);
    std::cout << "Compute reference ORB done!" << std::endl;

    // second image
    std::vector<std::vector<cv::KeyPoint>> keypoints_2;
    std::vector<std::vector<DescType>> descriptors_2;
    ComputeORBMultiLayer(img_2, keypoints_2, descriptors_2);
    std::cout << "Compute query ORB done!" << std::endl;
    ...
    BfMatchMultiLayer(descriptors_1, descriptors_2, matches);
    ...
    // Plot the matches
    cv::Mat image_show;
    cv::drawMatches(img_1, keypoints_1[0], img_2, keypoints_2[0], matches, image_show);
    cv::imshow("Matches", image_show);
    cv::imwrite("../results/matches_ORBMultiLayer.png", image_show);
    cv::waitKey(0);
...
// ORB pattern, 256 points, to compute descriptors
...
void ComputeORB(const cv::Mat &img, std::vector<cv::KeyPoint> &keypoints,
        std::vector<DescType> &descriptors, const double &scale) {

    const int half_patch_size = 8 * scale;
    const int half_boundary = 16 * scale;
    int bad_points = 0;
    ...
    for (auto &kp : keypoints) {
        ...
        /**
         * Initialization
         * 8 unsigned int values with 32 bits
         * each bit corresponds to a pair of random points
         * desc = {0000 0000 0000 0000 0000 0000 0000 0000,
        ...
        // for each point (1/256)
        for (int i = 0; i < 8; i++) {
            uint32_t d = 0; // 0000 0000 0000 0000 0000 0000 0000 0000
            ...
            for (int j = 0; j < 32; j++) {
                ...
                /**
                 * obtain rotated coordinate for 256 points
                 * axes rotation, yaw
                 * [x, y] -> [x', y']
                 *
                ...
                if (img.at<uchar>(pp.y, pp.x) < img.at<uchar>(qq.y, qq.x)) {
                    d |= 1 << j; // d = d|(1<<j), change 0 to 1 for the position j
                }
            }
            desc[i] = d;
        }
...
void ComputeORBMultiLayer(const cv::Mat &img, std::vector<std::vector<cv::KeyPoint>> &keypoints,
                     std::vector<std::vector<DescType>> &descriptors) {
    ...
    for (int i = 0; i < pyramids; ++i) {
        if (i == 0) {
            ...
            pyr.push_back(img);
            // keypoints
            keypoints.push_back(keypoints_org);
            // descriptor
            std::vector<DescType> descriptors_org;
            ComputeORB(img, keypoints_org, descriptors_org, scales[i]);
            descriptors.push_back(descriptors_org);
        } else {
            // image
            cv::Mat img_pyr;
            ...
            // scaled keypoints
            std::vector<cv::KeyPoint> kp_pyr;
            for (auto &kp : keypoints_org) {
                kp.pt *= scales[i];
                kp_pyr.push_back(kp);
            }
            keypoints.push_back(kp_pyr);
            // descriptor
            std::vector<DescType> desc_pyr;
            ComputeORB(img_pyr, kp_pyr, desc_pyr, scales[i]);
            descriptors.push_back(desc_pyr);
        }
    }
}
...
void BfMatch(const std::vector<DescType> &desc1, const std::vector<DescType> &desc2, std::vector<cv::DMatch> &matches) {
    ...
    // match each keypoint for desc1 and desc2
...
void BfMatchMultiLayer(const std::vector<std::vector<DescType>> &desc1,
    ...
    if (desc1.size() > 0 && desc2.size() > 0) {
        for (int i = 1; i < desc1.size(); ++i)
            n1 = desc1[i].size() > n1 ? desc1[i].size() : n1;
